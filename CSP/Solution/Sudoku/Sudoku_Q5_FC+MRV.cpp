...
bool checkOneCell(int row, int col);
...
bool checkZeroCellAlternative(int row, int col, int num)
{
	...
	for (int j = 0; j < N; j++)
	{
	if (!mrvCell[i][j].lock && mrvCell[i][j].countValues == 0)
	return true;
	}
	return false;*/
}
...
{
	for (int i = 0; i < N; i++)
		if (i != col && !mrvCell[row][i].lock && mrvCell[row][i].countValues == 1)
		{
			int num = mrvCell[row][i].values[0];
			Table[row][i] = num;
			initMrvTable();
			if (checkZeroCell(row, i))
			{
				Table[row][i] = 0;
				initMrvTable();
				return false;
	...
	for (int i = 0; i < N; i++)
		if (i != row && !mrvCell[i][col].lock && mrvCell[i][col].countValues == 1)
		{
			int num = mrvCell[i][col].values[0];
			Table[i][col] = num;
			initMrvTable();
			if (checkZeroCell(i, col))
			{
				Table[i][col] = 0;
				initMrvTable();
				return false;
	...
	for (int i = 0; i < N / 3; i++)
	{
		for (int j = 0; j < N / 3; j++)
		{
			if ((i + startRow == row) && (j + startCol == col))
				continue;
			if (!mrvCell[i + startRow][j + startCol].lock && mrvCell[i + startRow][j + startCol].countValues == 1)
			{
				int num = mrvCell[i+startRow][j+startCol].values[0];
				Table[i + startRow][j+startCol] = num;
				initMrvTable();
				if (checkZeroCell(i + startRow, j + startCol))
				{
					Table[i + startRow][j + startCol] = 0;
					initMrvTable();
					return false;
				}
				Table[i + startRow][j + startCol] = 0;
...
bool sudokuSolver()
{
	...
	for (int k = 0; k < mrvCell[row][col].values.size(); k++)
	{
		...
		if (isSafe(row, col, num))
		{
			...
			if (checkZeroCell(row, col))
			{
				Table[row][col] = 0;
				updateNeighborsAfterZeroCell(row, col, num);
				return false;
			}
